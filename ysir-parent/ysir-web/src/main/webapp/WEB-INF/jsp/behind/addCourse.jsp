...
<head>
    ...
    <script type="text/javascript">
        ...
        //页面加载完毕之后就执行以下代码片段
        $(function () {
            var subjectId = '${course.subject_id}';
            ...
            <div class="col-sm-10">
                <div class="input-group">
                        ...
                        <ul id="selectSubject" class="dropdown-menu">
                            <c:forEach items="${subjectList}" var="subject">
                                <li value='${subject.id}'><a href="#"
                                                             onclick="showName(this,'${subject.id}',1)">${subject.subject_name}</a>
                                </li>
                            </c:forEach>
                    ...
                    <c:if test="${empty course.subject_id}">
                        <input type="hidden" class="form-control" id="subjectId" name="subject_id" value="0">
                    </c:if>
                    <c:if test="${not empty course.subject_id}">
                        <input type="hidden" class="form-control" id="subjectId" name="subject_id"
                               value="${course.subject_id}">
                    </c:if>
                    <input type="text" class="form-control" disabled id="subject_name" aria-label="...">
                </div><!-- /input-group -->
            </div>
        </div>
